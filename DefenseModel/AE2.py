...
ap.add_argument("-t", "--target", default='/home/featurize/zwk/Datasets/Train',
                help="path to the target folder")
ap.add_argument("-v", "--val", default='/home/featurize/zwk/Datasets/test_PGD',
                help="path to the val folder")
ap.add_argument("-y", "--valtarget", default='/home/featurize/zwk/Datasets/Test',
                help="path to the val target folder")
ap.add_argument("-n", "--epochs", default=1000,
                help="epochs for train")
ap.add_argument("-b", "--batchsize", default=10,
                help="batch size for train")
ap.add_argument("-r", "--resume", default='./model/AE.pth',
...
def val(model):
    dst = labelAEDataLoader(args["val"].split(','), args["valtarget"].split(','), imgSize)
    testloader = DataLoader(dst, batch_size=1, num_workers=1)
    loss_func = nn.MSELoss()
    val_loss = []
    for i, (XI, YI, labels, ims) in enumerate(testloader):
        if use_gpu:
            x = Variable(XI.cuda(0))
            y = Variable(YI.cuda(0))
        else:
            x = Variable(XI)
            y = Variable(YI)
...
for epoch in range(Epochs):
    ...
    for i, (XI, YI, labels, ims) in enumerate(trainloader):
        if use_gpu:
            x = Variable(XI.cuda(0))
            y = Variable(YI.cuda(0))
        else:
